...
//    PyObject* stripped = Py_StripOpenMMUnits(obj);
    PyArrayObject* stripped = (PyArrayObject*) Py_StripOpenMMUnits(obj);
    PyObject* item = NULL;
    PyObject* item1 = NULL;
...
//            int type = PyArray_TYPE(reinterpret_cast<const PyArrayObject*>(stripped));
            int length = PyArray_SIZE(stripped);
//            int length = PyArray_SIZE(reinterpret_cast<const PyArrayObject*>(stripped));
...
//    PyObject* iterator = PyObject_GetIter(stripped);
    PyObject* iterator = PyObject_GetIter(reinterpret_cast<PyObject*>(stripped));
    if (iterator == NULL) {
        Py_DECREF(stripped);
        return SWIG_ERROR;
    }
...
//            int type = PyArray_TYPE(reinterpret_cast<const PyArrayObject*>(stripped));
//            int length = PyArray_DIM(reinterpret_cast<const PyArrayObject*>(stripped), 0);
            void* data = PyArray_DATA((PyArrayObject*) stripped);
            if (type == NPY_DOUBLE) {
                out.resize(length);
...
//    iterator = PyObject_GetIter(stripped);    // new reference
    iterator = PyObject_GetIter(reinterpret_cast<PyObject*>(stripped));
