...
#===================================================== SHAKE
 ...
 if (implicitSolvent==1):
  ...
  system=psf.createSystem(params,
                         nonbondedMethod=nbondMethod, nonbondedCutoff=cutoff*u.angstrom, switchDistance=switchdist*u.angstrom,
                         constraints=cons, removeCMMotion=False, rigidWater=rigidWater, #hydrogenMass=hmass*u.amu,
                         verbose=True);
...
#========================================== RLES via ALCH
 if (rles):
  if (alchcol==1): # beta
   dprint("Multicopy atoms are marked in the beta column of PDB file '"+alchfile+"'");
  elif (alchcol==2): #occupancy
   dprint("Multicopy atoms are marked in the occupancy column of PDB file '"+alchfile+"'");
  ...
  for o, b  in zip(alchpdb.occupancy, alchpdb.temperature_factor) :
   ...
   ialch+=1;
  dprint("Found ",len(alchatoms)," RLES atoms");
  ligand_atoms=alchatoms;
  dprint("Creating alchemical system");
  factory=alchsys(system, ligand_atoms=ligand_atoms, annihilate_sterics=1, annihilate_electrostatics=1, 
                  softcore_alpha=0, softcore_beta=0, softcore_a=1, softcore_b=0, softcore_c=1, softcore_d=1, softcore_e=1, softcore_f=1);
# reference lambda
  lambda0=1.0/rles_nrep
  dprint("Defining reference alchemical state with lambda=", lambda0);
  astate0=alch();
  astate0['lambda_restraints']=1.0
  astate0['lambda_electrostatics']=lambda0
  astate0['lambda_sterics']=lambda0
