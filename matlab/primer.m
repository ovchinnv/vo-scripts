function newmsa=primer(seqs, muts, basename, noalign)
if ~exist('noalign') ; noalign=0; end
if (~noalign)
...
i0=-1;
for i=1:numel(msa)
  inds=strfind(msa(i).Header,basename);
  if ~isempty(inds)
   i0=i
   break
  end
...
% now remove dashes from newmsa that were put in by the alignment
for i=1:numel(newmsa)
 newmsa(i).Sequence = strrep(newmsa(i).Sequence,'-','');
end
%
