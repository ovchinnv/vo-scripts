...
addpath('../')
% in this version, we limit analysis to a particular type(s) ; oherwise identical to pcastem
%
% the types you want to include
hatypes=[1];
hatypes=[1 2 5 6 8 9 11 12 13 16 17 18]; % group A
...
for hatype = hatypes;
 ...
 hasel=[hasel, 'hatype=''H',num2str(hatype),''''];
 flag=[flag,'-',num2str(hatype)];
end
...
% head
 aa='VTVTHSVNLLEN';
 idel0=strfind(ref.sequence,aa) + length(aa) ;
 aa='VHPVTIGECP';
...
%
 ...
 cmd= ['select rowid, *, length(sequence) as slen from ',table,' where Nunk=0 and sequence not LIKE ''%J%'' and (', hasel,')'] ; % ignore sequences with 'J' because Matlab does not score J's
% cmd
...
%
 ...
 save(['pca',num2str(tab),flag,'stem.mat'],'strains','msa')
