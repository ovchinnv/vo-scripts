...
if nargout > 1
     error(message('bioinfo:pdbwrite:TooManyOutputs', mfilename));
end

if nargin == 1 % a pdbstruct is expected and the output is a PDB-formatted string
...
%=== Check whether it is an old PDB structure
...
no_of_chains = length(m_seqres); % Number of chains
...
for i = 1:noOfFormula
    ...
    y(:,1:16) = repmat(['FORMUL  ',...
        ...
        mRec(i).hetID)],...
        noOfLines,1);
    %first line - check if hetID is 'HOH' or 'UNX'
...
for i = 1:noOfJournalEntries
    ...
    if(~isempty(mJournal(i).Title))  %Optional
    ...
    if(~isempty(mJournal(i).Editor))  %Optional
    ...
    if(~isempty(mJournal(i).Publisher)) %Optional
...
switch recName
    ...
    case 'OBSLTE'
        ...
        z(1,12:lineLen+31) = [mRec.repDate ' ' mRec.idCode '      ' mRec.rIdCode(1,:)];  %first line
        if(noOfLines > 1)
            z(2:end,9:10) = reshape(sprintf('%2d',2:noOfLines),2,noOfLines-1)';
...
for i = 1:noOfRemark1
    ...
    if(~isempty(mRec(i).Author)) %Optional AUTHOR
    ...
    if(~isempty(mRec(i).Title)) %Optional TITLE
    ...
    if(~isempty(mRec(i).Editor)) %Optional EDIT
    ...
    if(~isempty(mRec(i).Publisher)) %Optional PUBL
...
noOfLines = length(mRec);
z = repmat(blanks(81),noOfLines,1);
% Geck 1110522: PDBREAD assigns a NaN to missing sense information, however
% PDB file format only uses two spaces. We allocate one extra space, then
% remove NaNs and finally delete the extra column in order to comply with
% the PDF file format.
for i = 1:noOfLines
    z(i,1:71) = ['SHEET  ' ...
        sprintf('%3d %3s%2d %3s %1s%4d%1s %3s %1s%4d%1s%3d %4s%3s %1s%4d%1s %4s%3s %1s%4d%1s',...
...
for i = start_count:end_count
    ...
    str=['TER   ',...
        ...
        x.resSeq,...
        x.iCode)];
    strl=min(size(atomRecords,2),length(str));
    atomRecords(i,1:strl) = str(1:strl);
    count = count +1;
end
