...
icheck= [ ~(cellfun('isempty',(strfind(segid,'NP'))) & cellfun('isempty',(strfind(segid,'SC'))) & cellfun('isempty',(strfind(segid,'ST')))...
          & cellfun('isempty',(strfind(segid,'H'))) & cellfun('isempty',(strfind(segid,'F')))) & (element~='H') ] ; % exclude hydrogens from checking for a little extra speed
xchk=xpdb(icheck);ychk=ypdb(icheck);zchk=zpdb(icheck);

nseg=60;
for iseg=0:nseg-1
 iss=num2str(iseg);
 isel=find(ismember(strtrim(segid),{['NP',iss], ['ST',iss], ['SC',iss], ['H1',iss], ['H2',iss], ['F',iss] }));
 xsolu=xpdb(isel);
 ysolu=ypdb(isel);
 zsolu=zpdb(isel);
 solvent_pdb=['WAT',iss,'.pdb'];
 watmol=solvate(xsolu,ysolu,zsolu,solvent_pdb,buf,xchk,ychk,zchk,['W',iss],3); % assume that a single pdb will be written
