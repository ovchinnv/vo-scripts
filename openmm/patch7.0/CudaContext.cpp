...
CudaContext::CudaContext(const System& system, int deviceIndex, bool useBlockingSync, const string& precision, const string& compiler,
    ...
    if (boxIsTriclinic) {
        ...
        compilationDefines["RESTORE_PERIODIC(pos)"] =";"; //VO 2018 not implemented (stub)
...
// VO 2018 : trying to hack barostat to avoid wrapping, in simple PBC only
        compilationDefines["APPLY_PERIODIC_TO_POS(pos)"] =
            "{"
            "pos.x -= floor(pos.x*invPeriodicBoxSize.x)*periodicBoxSize.x; \\\n"
            "pos.y -= floor(pos.y*invPeriodicBoxSize.y)*periodicBoxSize.y; \\\n"
            "pos.z -= floor(pos.z*invPeriodicBoxSize.z)*periodicBoxSize.z;}";
        compilationDefines["RESTORE_PERIODIC(newpos, oldpos)"] =
            "{"
            "newpos.x += floor(oldpos.x*invPeriodicBoxSize.x)*periodicBoxSize.x; \\\n"
            "newpos.y += floor(oldpos.y*invPeriodicBoxSize.y)*periodicBoxSize.y; \\\n"
            "newpos.z += floor(oldpos.z*invPeriodicBoxSize.z)*periodicBoxSize.z;}";
        compilationDefines["APPLY_PERIODIC_TO_POS_WITH_CENTER(pos, center)"] =
            "{"
