...
c     COMMON /CBLKT/  NPP        ,K          ,EPS        ,CNV        ,
c    1                NM         ,NCMPLX     ,IK
      COMMON /CBLKT/  EPS        ,CNV        ,NPP        ,K          ,
     1                NM         ,NCMPLX     ,IK
c
...
c
  ...
  109 sgn = -1.d0
      cbp(1) = cmplx(bsrh(xl,bh(1),iz,c,a,bh,psgf,sgn),0.d0)
      is = 2
  110 if = iz-1
  ...
  114 sgn = 1.d0
      cbp(iz) = cmplx(bsrh(bh(iz),xr,iz,c,a,bh,psgf,sgn),0.d0)
      if = iz-2
...
c
  117    sgn = 1.d0
         cbp(ig) = cmplx(bsrh(bh(ig),xm,iz,c,a,bh,psgf,sgn),0.d0)
         sgn = -1.d0
         cbp(ig+1) = cmplx(bsrh(xm,bh(ig+1),iz,c,a,bh,psgf,sgn),0.d0)
         go to 136
c
c     case of a multiple zero
c
  118    cbp(ig) = cmplx(xm,0.d0)
         cbp(ig+1) = cmplx(xm,0.d0)
         go to 136
c
...
c
  ...
  122    f = (1.d0,0.d0)-fsg-hsg
         go to 124
  123    f = (1.d0,0.d0)+fsg+hsg
  124    i3 = 0
...
C      if (cdabs(cbp(n))-cdabs(cbp(1))) 137,142,139
  137 nhalf = n/2
      do 138 j=1,nhalf
         nt = n-j
         cx = cbp(j)
...
C         if (dimag(cbp(j))) 143,140,143
  ...
  102 psgf = 1.d0-fsg-hsg
      return
  103 psgf = 1.d0+fsg+hsg
      return
      end
...
c
         if (h .eq. 0.0d0) go to 108
         if (abs(e2(l)) .le. abs(c/h)) go to 108
         e2(l) = h*e2(l)
         if (e2(l) .ne. 0.0d0) go to 105
  108    p = d(l)+f
c
c     ********** order eigenvalues **********
c
...
c
      ...
      eps = 1.d0
  101 eps = eps/10.d0
      call dstore (eps+1.d0)
      if (v-1.d0) 102,102,101
  102 depmach = 100.d0*eps
      return
      end
      subroutine dstore (x)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
