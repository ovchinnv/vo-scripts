...
% use Simpson's Rule
%
% integrate in the reverse direction
...
for i=1:niter
 for j=1:ncv
  r(j,:,i)=cv(j,end:-1:1); % replicate cv data (cv's do not change in this calc.)
  f(j,:,i)=fc(row,end:-1:1); row=row+1;
 end
end
read=0;
end
...
work=work(rep,:)/6;
work=work-ones(ceil(m/2)+1-mod(m,2),1)*work(end,:);
rep=m+1-rep;
%return;
...
fe=zeros(nsample,ceil(m/2)+1-mod(m,2));
j=1;
for i=ib:bsize:ie
